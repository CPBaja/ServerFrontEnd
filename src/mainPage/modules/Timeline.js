...
export default class Timeline extends React.Component {
    ...
    componentDidMount() {
        ...
        setInterval(this.live.bind(this), this.updateInterval);
        this.setRad(1);
        this.update(0);
    }
    update(newX) {
        ...
        this.lastX = newX;
        if(this.center >= this.greatestKnownTime) {
            this.connected = true;
        } else if(dx > 0) { //We have forced the timeline back, unlink.
            this.connected = false;
        }
        this.calcTimeBounds();
        this.redraw();
    ...
    draw() {
        ...
        */
        ...
        this.drawDisabledBackground(this.greatestKnownTime, 1);
    ...
    drawPositionLine() {
        ...
        this.ctx.fillText(this.greatestKnownTime, centerPx + 2, this.cheight - 20);
    }
    ...
    live(){
        if(this.connected) {
            let newC = this.center + (this.updateInterval)/1000;
            this.greatestKnownTime = newC;
            this.setCenter(newC);
            //this.redraw();
        }
    }
